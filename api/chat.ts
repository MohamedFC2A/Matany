...
15. [التنفيذ الحرفي لطلب المستخدم واستوديو التوليد البصري العصبي]:
    - [الالتزام الحرفي الصارم بطلب المستخدم - Strict User Instruction Fidelity]: نفّذ طلب المستخدم بدقة مطلقة 100% كما طلبه تماماً، ويُحظر إضافة عناصر أو أفعال لم يطلبها، أو تفعيل أدوات لم يُشر إليها، ولا تفترض أي سلوك خارج نص وسياق طلب المستخدم الصريح.
    - يُحظر تماماً وبشكل قاطع توليد كود SVG إلا إذا ذكر المستخدم صراحة كلمة (SVG، كود SVG، فيكتور، متجهات، vector).
    - عند طلب المستخدم تصميم أو إنشاء أو توليد أو تعديل صورة (مثل: "صمم صورة...", "ارسم لي...", "انشئ صورة...", "توليد صورة...", "صورة كذا..."):
      بعد شرح توضيحي مقتضب من سطرين بالعربية، أخرج فوراً ومباشرة كتلة المعالجة العصبية \`\`\`neural-image ... \`\`\` لتوليدها عبر محرك Fathom QP3 العصبي للرؤية والتوليد البصري الفائق.`;
...
0. [الالتزام الحرفي الصارم والتام بطلب المستخدم - Strict User Instruction Fidelity]:
   - نفّذ طلب المستخدم بدقة مطلقة 100% كما طلبه تماماً، التزم بكافة القيود المطلوبة حرفياً دون زيادة أو نقصان.
   - يُحظر تماماً إضافة عناصر، أو اختراع وظائف، أو توليد أكواد (مثل SVG)، أو تفعيل بطاقات لم يطلبها المستخدم صراحة في رسالته الحالية.
    ...
    if (OPENROUTER_API_KEY) {
      try {
        ...
        if (visionRes.ok) {
          ...
          const errText = await visionRes.text().catch(() => '');
          console.warn('[Vision Extraction OpenRouter Muse Spark 1.2 Error]:', visionRes.status, errText);
        }
      } catch (backupErr) {
        console.warn('[Vision Extraction OpenRouter Muse Spark 1.2 Exception]:', backupErr);
      }
    }

    // 3. OpenRouter Gemini 2.5 Flash Vision Emergency Fallback
    if (OPENROUTER_API_KEY) {
      try {
        ...
        if (visionRes.ok) {
          ...
          const errText = await visionRes.text().catch(() => '');
          console.warn('[Vision Extraction Gemini Flash Fallback Error]:', visionRes.status, errText);
        }
      } catch (geminiErr) {
        console.warn('[Vision Extraction Gemini Flash Exception]:', geminiErr);
      }
    }
    return '';
  } catch (err) {
...
ممنوع منعاً باتاً كتابة أي تفكير باللغة الإنجليزية أو استخدام كود بلوك thought للإجابة.`;
  ...
  if (allExtractedUrls.length > 0) {
    ...
    if (willSearch) {
      ...
      console.log(`[FATHOM SEARCH PIPELINE Edge] OpenRouter Native Web Search Server Tool activated for Topic: "${activeSearchTopic.slice(0, 80)}..."`);
...
- حظر التخمين والهلوسة ورفض الإجابة وطلب الإذن (Strict Anti-Hallucination, Anti-Refusal & Zero Tool-Syntax Leaks):
  ...
  * قدّم الإجابة المحدثة والنهائية والنتائج الرياضية المباشرة فوراً للمستخدم باللغة العربية الفصحى مع التوثيق الموثوق.`;
  ...
  if (hasMultimodal || isVision) {
    ...
    if (DEEPSEEK_API_KEY) {
      candidateGateways.push({
        name: 'DeepSeek Chat Text & Multimodal Fallback (deepseek-chat @ api.deepseek.com)',
    ...
    if (DEEPSEEK_API_KEY) {
      candidateGateways.push({
        name: 'DeepSeek Chat Multimodal Text Synthesizer (deepseek-chat @ api.deepseek.com)',
        url: `${DEEPSEEK_BASE_URL}/chat/completions`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${DEEPSEEK_API_KEY}`,
    ...
    if (DEEPSEEK_API_KEY) {
      candidateGateways.push({
        name: 'DeepSeek Reasoner Sovereign Cyber Core (deepseek-reasoner @ api.deepseek.com)',
        url: `${DEEPSEEK_BASE_URL}/chat/completions`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${DEEPSEEK_API_KEY}`,
      ...
      candidateGateways.push({
        name: 'DeepSeek Chat Sovereign Cyber Backup (deepseek-chat @ api.deepseek.com)',
        url: `${DEEPSEEK_BASE_URL}/chat/completions`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${DEEPSEEK_API_KEY}`,
    ...
    if (DEEPSEEK_API_KEY) {
      candidateGateways.push({
        name: 'DeepSeek Chat Sovereign Persona Engine (deepseek-chat @ api.deepseek.com)',
        url: `${DEEPSEEK_BASE_URL}/chat/completions`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${DEEPSEEK_API_KEY}`,
    ...
    // General Text Chat Mode (Quant 3 Flagship Default - Primary: OpenRouter meta/muse-spark-1.3-contributor, Sovereign Core: DeepSeek)
    if (OPENROUTER_API_KEY) {
      candidateGateways.push({
    ...
    if (DEEPSEEK_API_KEY) {
      candidateGateways.push({
        name: 'DeepSeek Reasoner Flagship Cognitive Core (deepseek-reasoner @ api.deepseek.com)',
        url: `${DEEPSEEK_BASE_URL}/chat/completions`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${DEEPSEEK_API_KEY}`,
      ...
      candidateGateways.push({
        name: 'DeepSeek Chat Flagship Conversational Core (deepseek-chat @ api.deepseek.com)',
        url: `${DEEPSEEK_BASE_URL}/chat/completions`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${DEEPSEEK_API_KEY}`,
  ...
  for (const gate of candidateGateways) {
    if (gate.url.includes('openrouter.ai') && openrouterExhausted) {
      console.log(`[Vercel Edge] Fast-skipping ${gate.name} (OpenRouter limit exhausted).`);
      continue;
    }
    ...
    try {
      ...
      let resp: Response;
      try {
        const effectivePayload = DynamicParameterTuner.sanitizeForGateway(gate.url, gate.payload);
      ...
      if (resp.ok && resp.body) {
                ...
                if (trimmed.startsWith('data:') && !trimmed.includes('[DONE]')) {
                  ...
                  try {
                    ...
                    if (parsed.usage) {
                      ...
                      console.log(`[AI GATEWAY TELEMETRY Edge] Prompt: ${totalPromptTokens} tokens | Cache Hit: ${hitTokens} tokens (${hitRatio}%) | Cache Miss: ${missTokens} tokens | Completion: ${parsed.usage.completion_tokens || 0} tokens`);
                    }
                      ...
                      for (const ann of rawAnnotations) {
                        if (ann && ann.type === 'url_citation' && ann.url_citation) {
                          ...
                          if (cite.url && !seenCitationUrls.has(cite.url)) {
                            ...
                            const encoder = new TextEncoder();
                            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ choices: [{ delta: { reasoning_content: citationText } }] })}\n\n`));
                          }
                        }
                      }
                    }
                    ...
                    if (reasoningChunk) {
                      ...
                      try {
                        // Advance DAG / cognitive state without terminating stream during thinking
                      ...
                      if (trimmedChunk.length > 30) {
                        if (trimmedChunk === lastSeenContentChunk) {
                          ...
                          if (consecutiveIdenticalChunkCount >= 8) {
                            ...
                            console.warn(`[Vercel Edge] ⚠️ True consecutive runaway degeneration loop detected (${consecutiveIdenticalChunkCount}x identical chunks). Terminating stream safely.`);
        ...
        if (gate.url.includes('openrouter.ai') && (resp.status === 402 || resp.status === 403 || lastErrorText.includes('Key limit exceeded') || lastErrorText.includes('credit limit') || lastErrorText.includes('account settings'))) {
          openrouterExhausted = true;
          console.warn('[Vercel Edge] ⚠️ OpenRouter unavailable/limit reached. Fast-failing remaining OpenRouter gateways to switch to DeepSeek immediately.');
        }
      }
    } catch (err: any) {
